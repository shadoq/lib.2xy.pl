...
public class StarField extends AbstractEffect{
	...
	public final void init(){
		...
		for (int i = 0; i < count; i++){
			spriteCoords[i * 2] = (float) Math.round(MathUtil.randomize(XY.width, XY.centerX));
			spriteCoords[i * 2 + 1] = (float) Math.round(MathUtil.randomize(XY.height, XY.centerY));
			starSpeed[i] = MathUtil.randomize(1, 0);
		}
	}
	...
	public void update(float effectTime, float sceneTime, float endTime, float procent, boolean isPause){
		...
		for (int i = 0, idx = 0; i < count; ++i, idx = idx + 2){
			...
			factor = starSpeed[i] * XY.deltaTime * speed;

			x = spriteCoords[idx];
			y = spriteCoords[idx + 1];

			if (x < startX || y < startY || x > endX || y > endY){
				x = (float) Math.round(startX + MathUtil.randomize(width, startCenterX));
				y = (float) Math.round(startY + MathUtil.randomize(height, startCenterY));
			}
	...
	public void render(Batch batch, float parentAlpha){
		for (int i = 0, idx = 0; i < count; ++i, idx = idx + 2){
			...
			batch.draw(Gfx.whiteTexture, spriteCoords[idx], spriteCoords[idx + 1], 1, 1);
		}
	}
