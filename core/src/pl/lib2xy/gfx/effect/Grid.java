...
public class Grid extends AbstractEffect{
	...
	public final void init(){
		...
		texture = ResourceManager.getTextureRegion(textureFileNameEffect);
		...
		final float width = XY.width;
		final float height = XY.height;
	...
	public void update(float effectTime, float sceneTime, float endTime, float procent, boolean isPause){
		...
		if (flagSpeed > 0){
			...
			angle = angle + XY.deltaTime * flagSpeed;
					...
					for (int x = 0; x < gridX; ++x){
						...
						switch (mode){
							default:
								...
								//
							case 0:
								xStart = MathUtil.fastSin(angle + (x) * multiplierX) * amplitudeX + ((x) * xStep + startX);
								xEnd = MathUtil.fastSin(angle + (x + 1) * multiplierX) * amplitudeX + ((x + 1) * xStep + startX);

								yStart = MathUtil.fastCos(angle + (y * multiplierY)) * amplitudeY + ((y) * yStep + startY);
								yEnd = MathUtil.fastCos(angle + (y + 1) * multiplierY) * amplitudeY + ((y + 1) * yStep + startY);
								break;
							//
							// Mode 1
							//
							case 1:
								xStart = MathUtil.fastCos(angle + x * multiplierX) * amplitudeX + (x) * xStep + startX;
								xEnd = MathUtil.fastCos(angle + (x + 1) * multiplierX) * amplitudeX + (x + 1) * xStep + startX;

								yStart = MathUtil.fastSin(angle + (y) * multiplierY) * amplitudeY + (y) * yStep + startY;
								yEnd = MathUtil.fastSin(angle + (y + 1) * multiplierY) * amplitudeY + (y + 1) * yStep + startY;
								break;
							//
							// Mode 2
							//
