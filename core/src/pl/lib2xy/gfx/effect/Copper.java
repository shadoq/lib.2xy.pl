...
public class Copper extends AbstractEffect{
	...
	private int textureSizeX = Cfg.proceduralTextureSize;
	private int textureSizeY = Cfg.proceduralTextureSize;
	...
	public void update(float effectTime, float sceneTime, float endTime, float procent, boolean isPause){
		...
		step2 = (int) (step2 + copperStep2)
			...
			float angle = (pos + step) / 360 * MathUtil.PI2;
			float angle2 = (pos2 + step2) / 360 * MathUtil.PI2;
	...
	public void getGuiDefinition(Array<String[]> guiDef){
		...
		{Lang.get("gradient_2a"), Lang.get("gradient_2b"), Lang.get("gradient_3"), Lang.get("gradient_4"), Lang.get("4_Black_Color"),
		 Lang.get("Alpha_Palette"), Lang.get("Alpha_Palette_2"), Lang.get("Alpha_Palette_3"), Lang.get("Alpha_Palette_4")};
		guiDef.add(
		new String[]{"colorMode", "colorMode", "LIST", Lang.get("gradient_2a"),
					 TextUtil.join(Arrays.asList(colorModeItems), ",")}
		);
	...
	public void setValues(String changeKey, ArrayMap<String, String> values){
		...
		if (values.get("colorMode").equals(Lang.get("gradient_2a"))){
			mode = 0;
		} else if (values.get("colorMode").equals(Lang.get("gradient_2b"))){
			mode = 1;
		} else if (values.get("colorMode").equals(Lang.get("gradient_3"))){
			mode = 2;
		} else if (values.get("colorMode").equals(Lang.get("gradient_4"))){
			mode = 3;
		} else if (values.get("colorMode").equals(Lang.get("4_Black_Color"))){
			mode = 4;
		} else if (values.get("colorMode").equals(Lang.get("Alpha_Palette"))){
			mode = 5;
		} else if (values.get("colorMode").equals(Lang.get("Alpha_Palette_2"))){
			mode = 6;
		} else if (values.get("colorMode").equals(Lang.get("Alpha_Palette_3"))){
			mode = 7;
		} else if (values.get("colorMode").equals(Lang.get("Alpha_Palette_4"))){
			mode = 8;
		}
