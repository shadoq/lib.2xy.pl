...
public class Fire implements ProceduralInterface{
	...
	public static void generate(final Pixmap pixmap, int stepWidth, int randFraction, int maxSpeed, int maxStart){
		...
		pixmap.drawPixel(0, height - 1, (colorPalette[temp][0] << 24) | (colorPalette[temp][1] << 16) | (colorPalette[temp][2] << 8) | 255);
		...
		pixmap.drawPixel(width - 1, height - 1,
						 (colorPalette[temp][0] << 24) | (colorPalette[temp][1] << 16) | (colorPalette[temp][2] << 8) | 255);

		for (int i = 1; i < width - 1; i++){
			index = (width * (height - 1) + i);
			fireLuminate[index] = ((lineZero[(i - 1)] + lineZero[i] + lineZero[(i + 1)]) / 3 + stepWidth + rand.nextInt(randFraction));
		...
		for (int y = 1; y < width; y++){
			...
			fireLuminate[index_a] = ((fireLuminate[index_c] + fireLuminate[index_d]) / 2 + stepWidth + rand.nextInt(randFraction));
			if (fireLuminate[index_a] > max){
				fireLuminate[index_a] = max;
			}
			temp = fireLuminate[index_a];
			pixmap.drawPixel(0, index, (colorPalette[temp][0] << 24) | (colorPalette[temp][1] << 16) | (colorPalette[temp][2] << 8) | 255);
		...
		for (int x = 1; x < width - 1; x++){
			for (int y = 1; y < height; y++){
				...
				((fireLuminate[index_b] + fireLuminate[index_c] + fireLuminate[index_d]) / 3 + stepWidth + rand.nextInt(randFraction));
				if (fireLuminate[index_a] > max){
					fireLuminate[index_a] = max;
				}
				temp = fireLuminate[index_a];
