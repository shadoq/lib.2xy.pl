...
import pl.lib2xy.constans.GuiType;
...
public class BaseShader{
	...
	private static ShaderLoader shaderLoader = new ShaderLoader();
	...
	public BaseShader(String shaderName, boolean hasColor, boolean hasNormal, int numTexCoords){
		...
		if (!shaderName.equalsIgnoreCase("") && !shaderName.equalsIgnoreCase("default") && !shaderName.equalsIgnoreCase("-= default =-")){
			...
			Log.debug("SimpleShader::ReadShader", "Read file shader: " + shaderFile);

			try {
				XmlReader.Element parse = ResourceManager.getXml(shaderFile);
				...
				if (XY.cfg.debug){
					Log.debug("FileShaderFactory::ReadShader", "Read shader: " + shaderName);
				}
			} catch (Exception ex){
				Log.error(BaseShader.class.getName(), "Read error XML Shader", ex);
				shaderName = "Error ...";
			}
		} else {
			vertexInit = "";
	...
	public void compileShader(){
		...
		vertexShader = shaderLoader.parse(vertexShader);
		fragmentShader = shaderLoader.parse(fragmentShader);
		...
		if (XY.cfg.debug){
			Log.logLines("DefaultShader::vertexProgram", vertexShader);
			Log.logLines("DefaultShader::fragmentShader", fragmentShader);
		}

		Log.debug("Shader::compileShader", "shader compile: " + compileLog);
	...
	protected void printUniforms(){
		for (ObjectMap.Entry<String, Paramets> parameter : uniforms){
			if (XY.cfg.debug){
				Log.debug("printUniforms", parameter.toString());
			}
		}
	}
