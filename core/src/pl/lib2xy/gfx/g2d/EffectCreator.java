...
public class EffectCreator{
	...
	final private static String logTag = "EffectCreator";
	final private static boolean logActive = true;
	...
	public static void inOut(EffectType type, Actor actorIn, Actor actorOut, float duration, final Group group,
		...
		switch (type){
			case CENTER_TO_OUT_CENTER:
				moveFadeIn(actorIn, 0, 0, XY.centerX, XY.centerY, duration, group, removeActor);
				moveFadeOut(actorOut, 0, 0, XY.centerX, XY.centerY, duration, group, removeActor);
				break;
	...
	private static void removeActor(Group group, Actor actor){
		if (group != null && actor != null){
			...
			if (group.removeActor(actor)){
				if (logActive){
					Log.debug(logTag, "Actor removed! name: " + actorName, 2);
				}
			} else {
				if (logActive){
					Log.debug(logTag, "Actor removed! name: " + actorName, 1);
				}
			}
		}
	}
