	...
	private Element parseNode(Element node, Table mainTable){
		...
		for (int i = 0; i < childCount; i++){
			...
			if (element.getName().equalsIgnoreCase("p")){
				...
				// tag: img
				//
			} else if (element.getName().equalsIgnoreCase("image")){
				String src = element.getAttribute("src", "");
				int width = element.getIntAttribute("width", 0);
				int height = element.getIntAttribute("height", 0);
				Image image = new Image(ResourceManager.getTexture(recourceDir.path() + "/" + src));
				if (width > 0){
				...
				int width = element.getIntAttribute("width", 0);
				int height = element.getIntAttribute("height", 0);
				Image image = new Image(ResourceManager.getTextureRegion(src));
				if (width > 0){
					image.setWidth(width);
				}
				if (height > 0){
				...
				int width = element.getIntAttribute("width", 0);
				int height = element.getIntAttribute("height", 0);
				Button button = GuiResource.button(ResourceManager.getTexture(src));
				if (width > 0){
