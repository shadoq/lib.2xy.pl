...
import pl.lib2xy.gfx.util.ColorUtil;
...
public class GuiForm{
			...
			if (strings != null){
				if (!strings[0].equalsIgnoreCase("") && !strings[1].equalsIgnoreCase("")){
					if (globalElemets.containsKey(strings[0])){
						Log.debug(TAG, "Set global present: key: " + strings[0] + " value: " + strings[1]);
	...
	public void addFileBrowser(String name, String label, String value, String directory, final ChangeListener changeListener){
		...
		Log.debug(TAG, "Add FileBrowser ... name: " + name + " value: " + value + " directory: " + directory);
	...
	public void addRandomButton(String name, String label, final String[] values, final ChangeListener changeListener){
		...
		ChangeListener localChangeListener = new ChangeListener(){
			...
			public void changed(ChangeEvent event, Actor actor){
				...
				Log.debug("S3GuiForm::RandomButtonChange", "", 2);
	...
	public void addSavePresentButton(String name, String label, final ChangeListener changeListener){
		...
		Log.debug(TAG, "Add SavePresentButton ... name: " + name);
	...
	public void addReadPresentButton(String name, String label, final ChangeListener changeListener){
		...
		Log.debug(TAG, "Add ReadPresentButton ... name: " + name);
	...
	public Table getTable(){
		...
		if (guiTable == null){
			...
			while (iterator.hasNext()){
				...
				switch (el.type){
					default:
						Log.debug("S3GuiForm::getTable()", "Not support GUI from " + el.type.name(), 1);
						break;
					case FLOAT_SLIDER:
						Log.debug("S3GuiForm::getTable()", "Get float slider: " + key, 1);
						...
						break;
					case BUTTON_RANDOM:
						Log.debug("S3GuiForm::getTable()", "Get BUTTON RANDOM: " + key, 1);
						...
						break;
					case BUTTON_RESET:
						Log.debug("S3GuiForm::getTable()", "Get BUTTON RESET: " + key, 1);
					...
					case FILE_BROWSER:
						//						if (Cfg.DEBUG){
						//							Log.debug("S3GuiForm::getTable()", "Get file browser: " + key, 1);
						//						}
						//						gui.addFileBrowserChange(el.label, el, "valueString", el.valueString, true, 1, el.changeListener);
						break;
					case SELECT_INDEX:
						Log.debug("S3GuiForm::getTable()", "Get select_index: " + key, 1);
						...
						break;
					case LIST_INDEX:
						Log.debug("S3GuiForm::getTable()", "Get list_index: " + key, 1);
						...
						break;
					case COLOR_SELECT:
						Log.debug("S3GuiForm::getTable()", "Get select color: " + key, 1);
						gui.addColorBrowserChange(el.label, el, "valueColor", el.valueColor, true, 1, el.changeListener);
						break;
					case SELECT_COLOR_INDEX:
						Log.debug("S3GuiForm::getTable()", "Get select color index browser: " + key, 1);
					...
					case BUTTON_GROUP:
						...
						break;
					case TEXT_FIELD:
						Log.debug("S3GuiForm::getTable()", "Get text field: " + key, 1);
						...
						break;
					case CHECKBOX:
						Log.debug("S3GuiForm::getTable()", "Get checkbox: " + key, 1);
					...
					case PRESENT_READ:
						//						if (Cfg.USE_PRESENT){
						//							if (Cfg.DEBUG){
						//								Log.debug("S3GuiForm::getTable()", "Get present_index: " + key, 1);
						//							}
						//							gui.addListIndex(el.label, el, "valueFloat", el.valueArrayString, (int) el.valueFloat, true, 1, el.changeListener);
						//						}
						break;
					case SIZE2D_EDITOR:
						//							Log.debug("S3GuiForm::getTable()", "Get size2d editor: " + key, 1);
					...
					case IMAGE_LIST:
						//						if (Cfg.DEBUG){
						//							Log.debug("S3GuiForm::getTable()", "Get image list: " + key, 1);
						//						}
						//						gui.addImageList(el.label, el, "valueArrayString", el.valueArrayString, true, 1, el.changeListener);
						break;
					case LABEL:
						//						if (Cfg.DEBUG){
						//							Log.debug("S3GuiForm::getTable()", "Get LABEL: " + key, 1);
						//						}
	...
	public XmlWriter save(XmlWriter writer){
		...
		try{
			...
			while (iterator.hasNext()){
				...
				Log.debug("S3GuiForm::save", "Save key: " + key + " type: " + el.type, 2);

				switch (el.type){
					default:
						Log.debug("S3GuiForm::save", "Unsupported type !!!  key: " + key + " type: " + el.type, 1);
						break;
					case BUTTON_RANDOM:
					case BUTTON_RESET:
					case PRESENT_SAVE:
					...
					case FLOAT_SLIDER:
						...
						break;
					case FILE_BROWSER:
						//						element.attribute(el.name, ResourceManager.getFileSaveName(el.valueString));
						break;
					case SELECT_INDEX:
						element.attribute(el.name, el.valueFloat);
						break;
			...
			Log.debug("S3GuiForm::save", ex.toString() + " Exception in " + this.getClass().getName());
			return writer;
		}
	}
	...
	public void read(XmlReader.Element xmlElement){
		...
		while (iterator.hasNext()){
			...
			Log.debug("S3GuiForm::read", "Read key: " + key + " type: " + el.type, 3);

			switch (el.type){
				default:
					break;
				case FLOAT_SLIDER:
					Log.debug("S3GuiForm::read", "value: " + setting.getFloatAttribute(key, 1.0f), 3);
					el.valueFloat = setting.getFloatAttribute(key, 1.0f);
					break;
				case FILE_BROWSER:
					Log.debug("S3GuiForm::read", "value: " + setting.getAttribute(key, ""), 3);
				...
				case COLOR_SELECT:
					...
					break;
				case SELECT_COLOR_INDEX:
					Log.debug("S3GuiForm::read", "value: " + setting.getFloatAttribute(key, 0.0f), 3);
				...
				case LIST_INDEX:
					...
					break;
				case TEXT_FIELD:
					Log.debug("S3GuiForm::read", "value: " + setting.getText(), 3);
					el.valueString = setting.getText();
					break;
				case CHECKBOX:
					Log.debug("S3GuiForm::read CHECKBOX", "name: " + key + " value: "
					+ setting.getBooleanAttribute(key, false), 3);
					el.valueBoolean = setting.getBooleanAttribute(key, false);
					break;
				case SIZE2D_EDITOR:
					el.valueX = (int) (setting.getFloatAttribute(key + "_x", 0.0f) * XY.width);
					el.valueY = (int) (setting.getFloatAttribute(key + "_y", 0.0f) * XY.height);
					el.valueWidth = (int) (setting.getFloatAttribute(key + "_width", 1.0f) * XY.width);
					el.valueHeight = (int) (setting.getFloatAttribute(key + "_height", 1.0f) * XY.height);
					Log.debug("S3GuiForm::read SIZE2D_EDITOR",
							  "name: " + key + " x:" + el.valueX + " y:" + el.valueY + " width: " + el.valueWidth + " height: " + el.valueHeight, 3);
					break;
				case IMAGE_LIST:
	...
	public void savePresent(){
		...
		try{
			FileHandle fileSaveHandle = ResourceManager.getFileHandle(presentDir + className + ".xml");
			...
			if (fileSaveHandle.exists()){
				FileHandle fileReadHandle = ResourceManager.getFileHandle(presentDir + className + ".xml");
			...
			Log.error("savePresent", " Exception in save present: ", ex);
		}
	}
	...
	public void readPresent(int index){
		...
		try{
			...
			Log.error("readPresent", " Exception in read present: ", ex);
		}
	}
	...
	private String[] readPresentList(){
		try{
			...
			if (fileReadHandle.exists()){
				...
				names[0] = Lang.get("select_custom");
				for (int i = 0; i < countElement; i++){
					XmlReader.Element child = mainElement.getChild(i);
			...
			Log.error("readPresentList", " Exception: ", ex);
		}
		return null;
	}
}
