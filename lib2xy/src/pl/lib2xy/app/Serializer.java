...
public class Serializer{
	...
	private static class SceneVOSerializer implements Json.Serializer<SceneVO>{
		...
		public SceneVO read(Json json, JsonValue jsonData, Class type){
			Log.debug("S3AppImplSerializer::read", "json: " + json);
		...
		public void write(Json json, SceneVO sceneVO, Class knownType){
			...
			Log.debug("S3AppSerializer::write", "json: " + json);
	 ...
	 */
	private static class S3AppSerializer implements Json.Serializer<SceneImpl>{
		...
		@Override
		public void write(Json json, SceneImpl s3App, Class knownType){
			Log.debug("S3AppSerializer::write", "json: " + json);

			json.writeObjectStart();
			json.writeValue("class", s3App.getClass().getName());
			json.writeValue("name", s3App.data.name);
			json.writeValue("backgroundColor", s3App.data.backgroundColor.toString());

			json.writeValue("transition", s3App.data.transition.toString());
			json.writeValue("transitionDuration", s3App.data.transitionDuration);
			json.writeValue("transitionInterpolation", s3App.data.transitionInterpolation.toString());

			json.writeValue("background", s3App.data.backgroundFileName);

			json.writeValue("music", s3App.data.music);
			json.writeValue("musicType", s3App.data.musicPlayType.toString());
	...
	public static void create(){
		...
		registerSerializer(Scene.class, new S3AppSerializer());
		registerSerializer(SceneImpl.class, new S3AppSerializer());
