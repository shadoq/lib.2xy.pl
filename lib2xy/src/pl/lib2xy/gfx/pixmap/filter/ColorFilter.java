...
package pl.lib2xy.gfx.pixmap.filter;
...
public class ColorFilter implements ProceduralInterface, FilterPixmapInterface{
	...
	public static void generate(final Pixmap pixmap, Color colorBase, int colorPercentRed, int colorPercentGreen, int colorPercentBlue, int brithness,
		...
		for (int y = 0; y < height; y++){
			for (int x = 0; x < width; x++){
				...
				pixmap.drawPixel(x, y, (r << 24) | (g << 16) | (b << 8) | (int) (a * alphamult + minalpha));
			}
		}
	}
	...
	public void random(final Pixmap pixmap){
		generate(pixmap, Color.WHITE, 255, 255, 255, (int) (Math.random() * 255), (int) (Math.random() * 255), (int) (Math.random() * 255), 127);
	}
}
