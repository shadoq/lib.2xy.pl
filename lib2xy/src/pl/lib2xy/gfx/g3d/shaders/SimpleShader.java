...
public class SimpleShader extends BaseShader{
	...
	private final static String vertexDefault = "" +
	"void main(){" + "\n" +
	"	gl_Position = projectionMatrix * modelViewMatrix * a_position;" + "\n" +
	"}" + "\n" +
	"\n\n";
	private final static String fragmentDefault = "" +
	"void main(){" + "\n" +
	"	vec2 position = gl_FragCoord.xy / iResolution.xy;" + "\n" +
	"	gl_FragColor = vec4(position, 0.5+0.5*sin(iTime),1.0);" + "\n" +
	"}" + "\n" +
	"\n\n";
	...
	protected void createDefaultUniforms(){
		...
		uniforms.put("iTime", new Paramets("iTime", GuiType.NONE, ValueType.FLOAT, ""));
		uniforms.put("iResolution", new Paramets("iResolution", GuiType.NONE, ValueType.VECTOR2, ""));
		uniforms.put("iPosition", new Paramets("iPosition", GuiType.NONE, ValueType.VECTOR2, ""));
