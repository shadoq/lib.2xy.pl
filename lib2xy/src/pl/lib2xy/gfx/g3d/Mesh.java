...
public class Mesh{
	 ...
	 * @param normal
	 */
	public Mesh(int vertices, boolean is2dOrtho, boolean texture, boolean color, boolean normal){
	...
	private void initMesh(){
		...
		try{
			...
			GuiResource.alertMemory(Lang.get("OutOfMemoryMesh"));
		}

		Log.log("S3Mesh:initMesh", "Buffer size: " + (vertices * allDataOffset) + " vertices: " + vertices
		+ " verticesCount: " + verticesCount + " allDataOffset: " + allDataOffset, 1);

		if (is2dOrtho){
			projectionMatrix.setToOrtho2D(0, 0, XY.width, XY.height);
		}

		//
		// Create shader
		//
		Log.debug("S3Mesh:initMesh", "Create default shader ...", 2);

//		shader = ResourceManager.getShaderProgram("");
//		shader = Gfx.defaultShader;
		...
		mesh = new com.badlogic.gdx.graphics.Mesh(true, vertices, 0, attributes.toArray(new VertexAttribute[attributes.size()]));
		isUpdate = false;

		Log.log("S3Mesh:initMesh", "dataOffset=" + allDataOffset + " vertices=" + vertices + " vetextBufferSize="
		+ vertexBuffer.length, 1);
		Log.log("S3Mesh:initMesh", "projectionMatrix=" + projectionMatrix + " transformMatrix=" + modelViewMatrix, 2);
	}

	public void setShader(String shaderName){
		shader = ResourceManager.getShaderProgram(shaderName);
		Log.log("S3Mesh:initMesh", "Shader compile: " + shader.getLog());
	}
	...
	public Model getModel(){
		...
		return getModel(this, 0, mesh.getVertexSize(), GL20.GL_TRIANGLES, mat);
	}
	...
	public void render(int primitiveType, Texture texture, float time){
		...
		//		shader.setResolutionShader(XY.width, XY.height);
	...
	public void render(int primitiveType, BaseShader shader, Texture texture, float time){
		...
		if (isBlend){
			XY.gl.glEnable(GL20.GL_BLEND);
			XY.gl.glBlendFunc(blendSrcFunc, blendDstFunc);
		} else {
			XY.gl.glDisable(GL20.GL_BLEND);
		}
