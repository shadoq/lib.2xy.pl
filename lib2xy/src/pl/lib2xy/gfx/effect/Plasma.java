...
public class Plasma extends AbstractEffect{
	...
	private int sizeX = Cfg.proceduralTextureSize;
	private int sizeY = Cfg.proceduralTextureSize;
	...
	public void update(float effectTime, float sceneTime, float endTime, float procent, boolean isPause){
		...
		for (int y = 0; y < sizeY; y++){
			for (int x = 0; x < sizeX; x++){
				...
				if (layer1){
					...
					// Layer 1
					//
					color = (MathUtil.fastSin((float) x / divX1 + startX1 + (float) y / divY1 + startY1) + 1.0f) * 0.5f;
				}
				...
				if (layer2){
					...
					//
					color =
					color + (MathUtil.fastSin((float) x / divX2 + startX2 - (float) y / divY2 + startY2) + 1.0f) * 0.5f;
				}
				if (layer3){
					...
					// Layer 3
					//
					xDistance = x - XY.centerX * (MathUtil.fastCos(startX3 / divY3) + 1.0f / 2);
					yDistance = y - XY.centerY * (MathUtil.fastSin(startY3 / divY3) + 1.0f / 2);
					distanceToTargetPoint = Math.sqrt((xDistance * xDistance) + (yDistance * yDistance));
					color = color + (MathUtil.fastSin((float) distanceToTargetPoint / divX3 + startX3) + 1.0f) * 0.5f;
				}
				if (layer4){
					//
					// Layer 4
					//
					xDistance2 = x - XY.centerX * (MathUtil.fastCos(startX4 / divY4) + 1.0f / 2);
					yDistance2 = y - XY.centerY * (MathUtil.fastSin(startY4 / divY4) + 1.0f / 2);
					distanceToTargetPoint2 = Math.sqrt((xDistance2 * xDistance2) + (yDistance2 * yDistance2));
					color = color + (MathUtil.fastSin((float) distanceToTargetPoint2 / divX4 + startX4) + 1.0f) * 0.5f;
				}
		...
		if (speedX1Mode == 0){
			startX1 = startX1 + XY.deltaTime * speedX1;
		} else {
			startX1 = startX1 - XY.deltaTime * speedX1;
		}
		...
		if (speedY1Mode == 0){
			startY1 = startY1 + XY.deltaTime * speedY1;
		} else {
			startY1 = startY1 + XY.deltaTime * speedY1;
		}
		...
		if (speedX2Mode == 0){
			startX2 = startX2 + XY.deltaTime * speedX2;
		} else {
			startX2 = startX2 - XY.deltaTime * speedX2;
		}
		...
		if (speedY2Mode == 0){
			startY2 = startY2 + XY.deltaTime * speedY2;
		} else {
			startY2 = startY2 + XY.deltaTime * speedY2;
		}
		...
		if (speedX3Mode == 0){
			startX3 = startX3 + XY.deltaTime * speedX3;
		} else {
			startX3 = startX3 - XY.deltaTime * speedX3;
		}
		...
		if (speedY3Mode == 0){
			startY3 = startY3 + XY.deltaTime * speedY3;
		} else {
			startY3 = startY3 + XY.deltaTime * speedY3;
		}
		...
		if (speedX4Mode == 0){
			startX4 = startX4 + XY.deltaTime * speedX4;
		} else {
			startX4 = startX4 - XY.deltaTime * speedX4;
		}
		...
		if (speedY4Mode == 0){
			startY4 = startY4 + XY.deltaTime * speedY4;
		} else {
			startY4 = startY4 + XY.deltaTime * speedY4;
		}
